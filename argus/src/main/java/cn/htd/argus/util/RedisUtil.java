...
public class RedisUtil {
	...
	public void expire(final String key, final int seconds) {
		Jedis jedis = null;
		try {
			LOGGER.info("enter class RedisUtil and the method is expire---------->"
					+ key);
			jedis = jedisPool.getResource();
			...
			StringWriter w = new StringWriter();
		    e.printStackTrace(new PrintWriter(w));
			LOGGER.error("redis expire 方法  异常 key="+key+" "+w.toString());
		} finally {
			returnResource(jedis);
		}
	}
	...
	public String lpop(final String key) {
		...
		public Long hdel(final String key, final String field) {
			...
			try {
				jedis = jedisPool.getResource();
				returnValue = jedis.hdel(key,field);
				LOGGER.info("进入RedisUtil hdel 方法---------->查询出的 key为:"+key);
			} catch (Exception e) {
				StringWriter w = new StringWriter();
			    e.printStackTrace(new PrintWriter(w));
				LOGGER.error("redis hdel 方法  异常 key="+key+" "+w.toString());
			} finally {
				returnResource(jedis);
			}
			return returnValue;
